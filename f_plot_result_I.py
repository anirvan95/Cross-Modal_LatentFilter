...
# ############################################## Panel A ###############################################################
logreg_class_mean = np.load(os.path.join('results', 'classification', 'logreg_accuracies.npy'))
logreg_class_stds = np.load(os.path.join('results', 'classification', 'logreg_stds.npy'))
logreg_fold_accuracies = np.load(os.path.join('results', 'classification', 'logreg_fold_accuracies.npy'))
...
bars = ax_a.bar(x_positions, logreg_class_mean, width=bar_width, color=methods_color_A, alpha=1.0,
                yerr=logreg_class_stds, capsize=2,  error_kw={
             'elinewidth': 0.75,
             'capthick': 0.75,
             'linestyle': 'dashed',
             'dash_capstyle': 'butt'
...
method_map_A = {
    'Baseline': logreg_fold_accuracies[0],
    'Joint': logreg_fold_accuracies[1],
    'wo-CM (V)': logreg_fold_accuracies[2],
    'wo-CM (T)': logreg_fold_accuracies[3],
    'wo-CM (V,T)': logreg_fold_accuracies[4],
    'w-CM (V)': logreg_fold_accuracies[5],
    'w-CM (T)': logreg_fold_accuracies[6],
    'w-CM (V,T)': logreg_fold_accuracies[7]
}
ref_method = 'w-CM (V,T)'
methods = list(method_map_A.keys())
test_results = {}
ref_scores = method_map_A[ref_method]
methods_to_test = []
raw_pvals = []
for m, scores in method_map_A.items():
    if m == ref_method:
        continue
    # Paired t-test
    stat, p = stats.ttest_rel(scores, ref_scores, nan_policy='omit')
...
tops = logreg_class_mean + logreg_class_stds
y_base = np.nanmax(tops)
y_step = 0.05
current_y = y_base + y_step

# Iterate in reverse order
for m in reversed(list(test_results.keys())):
    res = test_results[m]
    p = res['p_value']
    stars = res['stars']

    if p < 0.05 and stars != 'ns':
        m_idx = methods.index(m)
        ref_idx = methods.index(ref_method)

        x_ref = x_positions[ref_idx]
        x_m = x_positions[m_idx]

        add_sig(ax_a, x_ref, x_m, current_y, stars, line_h=0.02)
        current_y += y_step
