...
gs = fig.add_gridspec(
    nrows=5,
    ncols=24,
    height_ratios=[1.15, 1.0, 0.65, 0.65, 0.65],  # one ratio per row
    hspace=0.8,
    wspace=0.5,
    left=0.075,
    right=0.99,
...
# ---------- Row 1 ----------
# Panel B
ax_b = fig.add_subplot(gs[0, 13:])

# Panel B A coluns
gs_b = gs[0, :12].subgridspec(nrows=1, ncols=3, wspace=0.1)
ax_a = [fig.add_subplot(gs_b[0, i]) for i in range(3)]

# ---------- Row 2 ----------
gs_c_left = gs[1, :12].subgridspec(nrows=1, ncols=3, wspace=0.1)
gs_c_right = gs[1, 12:].subgridspec(nrows=1, ncols=3, wspace=0.1)
...
gs_bottom = gs[2:, :].subgridspec(nrows=3, ncols=6, wspace=0.1, hspace=0.1)
...
for i in range(0, len(target_tac_outputs)):
    ...
    for prop in range(0, len(tac_properties)):
        ax_a[prop].plot(time_s, mean_error[:, prop], color=methods_color[i], label=methods_label[i], linewidth=1.0, linestyle='solid')
        ax_a[prop].fill_between(time_s, mean_error[:, prop] - std_error[:, prop]/10,  mean_error[:, prop] + std_error[:, prop]/10, color=methods_color[i], alpha=0.1)
        ax_a[prop].set_title(tac_properties[prop], fontsize=10)
        ax_a[prop].set_xlabel('Time (s)', fontsize=10)
        ax_a[prop].grid(True, linestyle='--', linewidth=0.4, alpha=0.5)
        if prop != 0:
            ax_a[prop].tick_params(labelleft=False)
...
import matplotlib.ticker as ticker
for i in range(0, 3):
    ax_a[i].set_yticks(tac_yticks)
    ax_a[i].set_ylim(tymin, tymax)
    ax_a[i].yaxis.set_major_formatter(two_decimal_formatter)

ax_a[0].set_ylabel('NMSE', fontsize=10)
handles = [plt.Line2D([0], [0], color=methods_color[i], lw=4, alpha=1.0, linestyle=(0, ())) for i in range(len(methods_label))]
legend = ax_a[1].legend(handles, methods_label, loc="upper center", bbox_to_anchor=(0.5, 1.4),
...
legend = ax_b.legend(handles, methods_label, loc="upper center", bbox_to_anchor=(0.5, 1.3),
                    ncol=len(methods_label), frameon=False,
                    columnspacing=0.9, handlelength=1.25, handletextpad=0.5, fontsize=10)
...
test_results = {
    m: {'pvals': np.ones(num_groups), 'symbol': ['ns'] * num_groups}
    for m in methods_label if m != ref_method
}
...
for prop_idx in range(num_groups):
    ...
    for m_idx, m in enumerate(methods_label):
        if m == ref_method:
            continue

        if prop_idx < len(vis_properties):
            x = np.mean(target_vis_outputs[ref_idx], axis=1)[:, prop_idx]
            y = np.mean(target_vis_outputs[m_idx], axis=1)[:, prop_idx]
        else:
            p = prop_idx - len(vis_properties)
            x = np.mean(target_tac_outputs[ref_idx], axis=1)[:, p]
            y = np.mean(target_tac_outputs[m_idx], axis=1)[:, p]
    ...
    _, pvals_holm, _, _ = multipletests(raw_pvals, alpha=0.05, method='holm')

    for m, p_corr in zip(methods_to_test, pvals_holm):
        test_results[m]['pvals'][prop_idx] = p_corr
        test_results[m]['symbol'][prop_idx] = p_to_stars(p_corr)
...
methods_label = [r'$\mathbf{y}^{T}$', r'CM-V2T', r'GT']
...
legend = ax_d[0, 2].legend(handles, methods_label, loc="upper center", bbox_to_anchor=(0.75, 1.375), ncol=len(methods_label),
                    columnspacing=0.9, frameon=False, handlelength=1.25, handletextpad=0.5, fontsize=10)


# Define which subplots to highlight
highlight_list = [(0, 0), (0, 3), (1, 1), (1, 4), (2, 2), (2, 5)]  # corners, for example

for i, j in highlight_list:
    for spine in ax_d[i, j].spines.values():
        spine.set_edgecolor('tomato')
        spine.set_linewidth(2)

# #####################################################################################################################
ax_b.text(-0.075, 1.275, r'{b}',
          transform=ax_b.transAxes,
          ha='right', va='center', fontsize=12)

ax_a[0].text(-0.45, 1.275, r'{a}',
...
ax_ref.annotate(
    '',
    xy=(3.25, -0.39),      # arrow end (to the right)
    xytext=(0.0, -0.39),  # arrow start
...
ax_ref.text(
    3.25/2, -0.39,  # Changed y-coordinate to match arrow position
    r'Aligned set',
